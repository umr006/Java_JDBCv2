...
public class MessagesRepositoryJdbcImpl implements MessagesRepository {
    ...
    private String FIND_ALL_SQL = """
            SELECT users.id, users.login, users.passwd, list_of_created_rooms.id, chatroom_owner_id, chatroom_id, chatroom_name
            ...
            ORDER BY users.id LIMIT %d OFFSET %d;
            """;
    ...
    public List<User> findAll(int page, int size) {
        ...
        FIND_ALL_SQL = String.format(FIND_ALL_SQL, page, page * size);
        try (PreparedStatement preparedStatement = connection.prepareStatement(FIND_ALL_SQL);
            Statement statementOfCreatedRooms = connection.createStatement();
            Statement statementOfUserSocialized = connection.createStatement()) {

            ResultSet resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
                ResultSet resultSetOfCreatedRooms = statementOfCreatedRooms.executeQuery(FIND_ALL_SQL);
                ResultSet resultSetOfUserSocialized = statementOfUserSocialized.executeQuery(FIND_ALL_SQL);

                userList.add(new User(
                        resultSet.getLong("id"),
                        resultSet.getString("login"),
                        resultSet.getString("passwd"),
                        UserCreatedRooms(resultSetOfCreatedRooms),
                        UserSocializedRooms(resultSetOfUserSocialized)));
            }
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
    ...
    private List<ChatRoom> UserSocializedRooms (ResultSet resultSet) throws SQLException {
        List<ChatRoom> userSocializedRoomList = new ArrayList<>();

        while (resultSet.next()) {
            userSocializedRoomList.add(new ChatRoom(resultSet.getLong("id"),
                    resultSet.getString("chatroom_name"),
                    resultSet.getLong("chatroom_owner_id"),
                    new ArrayList<>()));
        }
        return userSocializedRoomList;
    }

    private List<ChatRoom> UserCreatedRooms (ResultSet resultSet) throws SQLException {
        List<ChatRoom> userSocializedRoomList = new ArrayList<>();

        while (resultSet.next()) {
            if(resultSet.getLong("id") == resultSet.getLong("chatroom_owner_id")) {
                userSocializedRoomList.add(new ChatRoom(resultSet.getLong("id"),
                        resultSet.getString("chatroom_name"),
                        resultSet.getLong("chatroom_owner_id"),
                        new ArrayList<>()));
